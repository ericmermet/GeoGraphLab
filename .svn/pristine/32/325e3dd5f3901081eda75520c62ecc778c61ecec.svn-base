 ...
 * 2007 - 2013
 *
 * Classifieur1D.java in fr.ign.cogit.geographlab.algo.legende
 * 
 */
package fr.ign.cogit.geographlab.algo.legende;
...
//		
//		Classifieur1D.EQUAL_RANGE = Messages.getString("ui.renderer.style.ColorThemingStylePanel.Equal-Interval");
//		Classifieur1D.EQUAL_NUMBER = Messages.getString("ui.renderer.style.ColorThemingStylePanel.Quantile-Equal-Number");
//		Classifieur1D.MEAN_STDEV = Messages.getString( "ui.renderer.style.ColorThemingStylePanel.Mean-Standard-Deviation");
//		Classifieur1D.MAX_BREAKS = Messages.getString("ui.renderer.style.ColorThemingStylePanel.Maximal-Breaks");	
//		Classifieur1D.JENKS_BREAKS = Messages.getString("ui.renderer.style.ColorThemingStylePanel.Jenks-Optimal-Method");	
//
//		//-- make a list for the GUI
//		List classifierList = new ArrayList();
//		classifierList.add(Classifieur1D.EQUAL_RANGE);
//		classifierList.add(Classifieur1D.EQUAL_NUMBER);
//		classifierList.add(Classifieur1D.MEAN_STDEV);
//		classifierList.add(Classifieur1D.MAX_BREAKS);
//		classifierList.add(Classifieur1D.JENKS_BREAKS);
...
//	}
     ...
     */
    public static double[] classificationIntervallesEgaux(double min, double max, int numberClasses){
...
//    			nbZero++;
//		}
//    	double[] tabReturn = {0.0,17.0,33.0,50.0,66.0,83.0, 100.0};
//    	if( nbZero == data.length )
//    		return tabReturn;
//    	
...
//        double max = DoubleArray.max(data);
    ...
    /**
     * calculates class limits with equal number, which is equal to the "quantiles" method.
     * Note that differences in the items per classes occur, if items have same values
     * and need to be grouped into the same class. 
     * @param data
     * @param numberClasses
     * @return break values for classes. E.g. for 4 ranges 3 breaks are returned. Min and Max Values are not returned.   
     */
    public static double[] classificationEffectifsEgaux(double[] data, int numberClasses){
    ...
    public static double[] classifyKMeansOnExistingBreaks(double[] data, int numberClasses, int initialLimitAlgorithm){
        ...
        double SDAM = Classifieur1D.calcSDAM(sortedData);
        ...
        if (initialLimitAlgorithm == 1){     
            tempLimits =  Classifieur1D.classifyMaxBreaks(sortedData, numberClasses);
        }
        else if(initialLimitAlgorithm == 2){
            tempLimits =  Classifieur1D.classificationIntervallesEgaux(DoubleArray.min(sortedData), DoubleArray.max(sortedData), numberClasses);
        }
        else if(initialLimitAlgorithm == 3){
            tempLimits =  Classifieur1D.classificationEffectifsEgaux(sortedData, numberClasses);
        }
        else if(initialLimitAlgorithm == 4){
            tempLimits =  Classifieur1D.classifyMeanStandardDeviation(sortedData, numberClasses);
		} 
        else if (initialLimitAlgorithm == 5) {
			tempLimits = Classifieur1D.classifyNaturalBreaks(sortedData,
					numberClasses);
        }        
        else{
            //=== Default ===
        ...
        limits = tempLimits;
        double GVF = Classifieur1D.calcGVF(sortedData, tempLimits, SDAM);
        //========== optimize ==============
        ...
        while(moveOn){
            ...
            tempLimits = Classifieur1D.adjustLimitsKMeans(sortedData, limits);
            //-- calc fit (i.e. GVF)
            double newGVF = Classifieur1D.calcGVF(sortedData, tempLimits, SDAM);
    ...
    public static double[] adjustLimitsKMeans(double[] data, double[] oldLimits){
        ...
        int[] oldClasses = Classifieur1D.classifyData(data, oldLimits);
        //-- calc class means         
        double[] means = Classifieur1D.calcClassMeans(data, oldClasses, numberClasses);
    ...
    public static int[] classifyData(double[] data, double[] limits){
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int j = 0; j < nClasses; j++) {
                isInClass = Classifieur1D.isInClass(data[i], finalLimits[j], finalLimits[j+1]);
                if (isInClass){
                    classes[i]=j;
                    assigned = true;
                }                
    ...
    public static double calcGVF(double[] data, double[] limits, double SDAM){
        ...
        int[] classes = Classifieur1D.classifyData(data, limits);
        //-- calc class mean values        
        double[] means = Classifieur1D.calcClassMeans(data, classes, numberClasses);
        //-- calc SDCM (squared deviations, class means)
        double SDCM = Classifieur1D.calcSDCM(data, classes, means, numberClasses);
        //-- calc Goodness of Variance fit (GVF)
        double GDF = Classifieur1D.calcGVF(SDAM, SDCM);
        return GDF;
    }
