...
public class Noeud extends ObjetTopologique implements Cloneable {
	...
	private int iD;
	private double ponderation = 10.0;
...
//	private final EventListenerList listenersSelectionNoeuds = new EventListenerList();
	...
	public Noeud(String nom, int iD) {
		this(nom);
		this.iD = iD;
		this.noeudGraphique = new NoeudGraphique(this, new GeometryFactory().createPoint(new Coordinate()));
	}
	...
	public Noeud(Point position, String nomIndicateur) {
		this(new String("V_" + new Integer((int) (100000 * Math.random())).toString()), nomIndicateur, position);
	}
