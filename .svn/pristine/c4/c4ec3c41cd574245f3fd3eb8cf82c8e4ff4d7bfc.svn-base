...
public class EloignementMoyenODSpace extends Thread {
	...
	public EloignementMoyenODSpace(Carte carte, int typeChemin) {
		this.nouvelleCartePourNouvelleCouche = new Carte(carte, Messages.getString("EloignementMoyenODSpace.0"), Messages.getString("EloignementMoyenODSpace.1") + new Date().getTime()); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void run() {
		...
		for (OD iterOD : espaceSet) {
			...
			eloignementMoyenDuNoeud = 	localPCC.getStartVertex().getValeurPourIndicateur(this.nouvelleCartePourNouvelleCouche.getNomIndicateurCourant()) +
										localPCC.getLongueur()/(this.espace.espaceDeDef.size());
