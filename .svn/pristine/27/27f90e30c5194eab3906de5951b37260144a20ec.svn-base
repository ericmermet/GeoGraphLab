...
public class MyHashSet<E> extends AbstractSet<E> implements Set<E>, Cloneable, java.io.Serializable {
	...
	public MyHashSet(Collection<? extends E> c) {
		this.map = new HashMap<E, Object>(Math.max((int) (c.size() / .75f) + 1, 16));
		addAll(c);
	}
	...
	public MyHashSet(int initialCapacity) {
		this.map = new HashMap<E, Object>(initialCapacity);
	}
	
	// private
	// * constructor is only used by LinkedHashSet.) The backing HashMap
	// instance
	// * is a LinkedHashMap with the specified initial capacity and the
	// specified
	// * load factor.
	// *
	// * @param initialCapacity
