...
public class CentraliteDePouvoir extends Thread {
	...
	public CentraliteDePouvoir(Carte carte) {
		this.nouvelleCartePourNouvelleCouche = new Carte(carte, "Centralite_De_Pouvoir", "CentraliteDePouvoir" + new Date().getTime());
	...
	public void run() {
		...
		for (Noeud iterNoeud : this.nouvelleCartePourNouvelleCouche.getGraphe().getNoeuds()) {
			...
			// En rapport avec le nombre de connexions possibles, pour un n il y a n-1 connexions possible
			Double centralitePouvoir = new Double(degre) / (nbOperation-1);
			
			this.max = Math.max(centralitePouvoir, this.max);
			this.min = Math.min(centralitePouvoir, this.min);
			
			iterNoeud.setIndicateurValeur(this.nouvelleCartePourNouvelleCouche.getNomIndicateurCourant(), new Double(centralitePouvoir));
