...
//	private static ArcEvent event(Arc n, ArcGraphique ng) {
...
//	}
	...
	public ArcGraphique(Arc a) {
		...
		setArcTopologique(a);
//		a.addSelectionArcListener(ArcGraphique.event(a, this));
//		a.addNomArcListener(ArcGraphique.event(a, this));
	...
	public void changePSource(Point nouveauPointSource) {
		setPSource(nouveauPointSource);
		if( this.geomLine != null )
			changePointExtremite(nouveauPointSource);
	}
	...
	public void changePCible(Point nouveauPointCible) {
		setPCible(nouveauPointCible);
		if( this.geomLine != null )
			changePointExtremite(nouveauPointCible);
	}
	...
	public void autoSetShape() {
		this.edgeShape = new Line2D.Double(
				getArcTopologique().getSource().getPosition().getX() + ConstantesApplication.vertexRadius / 2,
				getArcTopologique().getSource().getPosition().getY() + ConstantesApplication.vertexRadius / 2,
				getArcTopologique().getTarget().getPosition().getX() + ConstantesApplication.vertexRadius / 2,
				getArcTopologique().getTarget().getPosition().getY() + ConstantesApplication.vertexRadius / 2);
	}
	...
	public void changePointExtremite(Point p) {
		
		Coordinate[] coordinates = this.geomLine.getCoordinates();
		
		//Inversion du Y
		p = new GeometryFactory().createPoint(new Coordinate(p.getX(), -p.getY()));
		
		if( p.distance(this.geomLine.getStartPoint()) < p.distance(this.geomLine.getEndPoint()) ) {
			//Le point start est le plus proche du nouveau point
			coordinates[0] = p.getCoordinate();
		}else{
			//Le point end est le plus proche du nouveau point
			coordinates[coordinates.length-1] = p.getCoordinate();
		}
		this.geomLine = new GeometryFactory().createLineString(coordinates);
	...
	public Geometry getGeomClone() {
		if( this.geomLine != null)
			return (Geometry) this.geomLine.clone();
		return null;
	}
